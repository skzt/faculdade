...
#
# 	#input()
with open(os.path.join(TEST_DIR, f"{MODEL}.csv"), "a") as file:
	fig = plt.figure()
	num = 0
	labels = []
	# for i in range(12):
	...
	for img_name in os.listdir(os.path.join(TEST_DIR, "test")):

		img = cv2.imread(os.path.join(TEST_DIR, "test", f"{img_name}"), cv2.IMREAD_GRAYSCALE)
		img = cv2.resize(img, (ROW_SIZE, COL_SIZE))
